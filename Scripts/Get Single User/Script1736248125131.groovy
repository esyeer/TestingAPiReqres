...
//set the restUrl and method
request.setRestUrl('https://reqres.in/api/users?id=1')
...
assert jsonResponse.data != "" : "Missing data"
assert jsonResponse.data.id == 1 : "expected id 1 but actuals (${jsonResponse.data.id})"
assert jsonResponse.data.email == 'george.bluth@reqres.in' : "expected id 'george.bluth@reqres.in' but actuals (${jsonResponse.data.email})" 
assert jsonResponse.containsKey("support") : "Response does not contain 'support' property"
assert jsonResponse.support != "" : "Missing support"
